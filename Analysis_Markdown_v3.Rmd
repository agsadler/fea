...
# Number of products with each certification type, by product and country
tab9a <- tab9_data %>%
  ...
  summarise(
    count = sum(!is.na(value) & value == 1, na.rm = TRUE),
    .groups = 'drop'
  )

# now I have a count per certification type... need to create columns for product name and certification name... 
# and figure out a way to turn this count into a ranking and arrange in descending order by the new product name
# and then rank and slice...


tab9c <- tab9_data %>%
    select(country, all_of(vector_certifcols)) %>%
    pivot_longer(
      cols = c(vector_certifcols),
      names_to = "certification_type",
      values_to = "value") %>%
    filter(!is.na(value)) %>%
    group_by(country, certification_type) %>%
...
tab9a <- left_join(
  # number of brands in each sentinel food
  tab9_data %>%
    select(country, vector_certifcols) %>%
    pivot_longer(cols = c(vector_certifcols), 
                 names_to = "food", 
                 values_to = "certifcols") %>%
#    filter(!is.na(brand)) %>%
    group_by(country, food) %>%
    summarise(n_certifcols = n()),
...
# Pivot the data so that each country becomes a separate column
tab9_combined_wide <- tab9_combined %>%
  pivot_wider(
    names_from = country,
    values_from = c(cert_count, cert_bac_count, cert_bac_perc, cert_eu_count, cert_eu_perc, 
                    cert_ibd_count, cert_ibd_perc, cert_io_count, cert_io_perc, cert_jb_count, 
                    cert_jb_perc, cert_ofg_count, cert_ofg_perc, cert_off_count, cert_off_perc, 
                    cert_pgsg_count, cert_pgsg_perc, cert_pgso_count, cert_pgso_perc, cert_pob_count, 
                    cert_pob_perc, cert_qwfc_count, cert_qwfc_perc, cert_sa_count, cert_sa_perc, 
                    cert_usda_count, cert_usda_perc)
  )
