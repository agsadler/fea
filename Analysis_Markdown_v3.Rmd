...
tab10_main <- tab10_data %>%
  ...
  summarise(
    theme_count = sum(!is.na(value) & value == 1, na.rm = TRUE), 
    org_count = sum(as.numeric(org_products_count), na.rm = TRUE),
    .groups = 'drop'
  ) %>%
...
## 2. TYPE OF PROMOTIONAL MATERIALS



## 3. PROMOTIONAL MATERIALS' THEMES
...
tab11b_main <- tab11_data %>%
  ...
  summarise(
    theme_count = sum(!is.na(value) & value == 1, na.rm = TRUE), 
    promo_count = sum(as.numeric(promo_count), na.rm = TRUE),
    .groups = 'drop' # something is going wrong at this point because Brazil should have 1 for quality and it doesn't here
  ) %>%
  mutate(perc = round((theme_count / promo_count)*100,2)) %>%
  select(country, theme, theme_count, perc)


# Tried a slightly different approach to see if the pivot with promo_count was the issue but it still doesn't work:
#tab11c_main <- tab11_data %>%
#  select(country, all_of(vector_themes_promo)) %>%
#  pivot_longer(
#    cols = all_of(vector_themes_promo),
#    names_to = "theme",
#    values_to = "value"
...
# THIS MAKES ME THINK TABLE 10 IS ALSO INCORRECTLY CALCULATING...


# Create the overall summary table - NOTE THAT THIS SEEMS TO WORK???
