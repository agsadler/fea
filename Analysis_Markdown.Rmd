...
writeData(table_outputs, sheet = "Table 3 - neighbourhood", x = tab3_neighbourhood_data)
saveWorkbook(table_outputs, "C:/Users/s1985751/Documents/GitHub/fea/table_outputs.xlsx", overwrite = TRUE)
...
writeData(table_outputs, sheet = "Table 3 - city", x = tab3_city_data)
saveWorkbook(table_outputs, "C:/Users/s1985751/Documents/GitHub/fea/table_outputs.xlsx", overwrite = TRUE)
```
...
tab5_country <- tab5_country_summary_stats %>%
  gt() %>%
  cols_label(
    country = "Country",
    count_org_rice_vendors = "Organic sample size (n)",
...
#### Table S1: City-level summary statistics

```{r echo = FALSE, message = FALSE, warning = FALSE}

# Calculate number of organic and conventional vendors, median, IQR, min and max price
# Filtered on vendors with both organic and conventional rice
tabs1_summary_stats <- rice_prices_data %>%
  filter(rice_price_both == 1) %>%
  group_by(country, city) %>%
  summarise(
    count_org_rice_vendors = sum(!is.na(rice_price_org_kg_usd)),
...
#### Table S1: Country-level summary statistics
...
# Calculate number of organic and conventional vendors, median, IQR, min and max price
# Filtered on vendors with both organic and conventional rice
tabs1_country_summary_stats <- rice_prices_data %>%
  filter(rice_price_both == 1) %>%
  group_by(country) %>%
  summarise(
    count_org_rice_vendors = sum(!is.na(rice_price_org_kg_usd)),
...
tabs1_country <- tabs1_country_summary_stats %>%
  gt() %>%
  cols_label(
    country = "Country",
    count_org_rice_vendors = "Organic sample size (n)",
