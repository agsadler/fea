...
tab2_neighbourhood <- tab2_neighbourhood_data %>%
  ...
  cols_label(
    ...
    org_vendor_perc = " ",
    median_org_products_count = "Organic sentinel foods, median (IQR)",
    iqr_org_products_count = " "
  ) %>%
  fmt_percent(
  ...
  cols_merge(
    columns = c("median_org_products_count", "iqr_org_products_count"),
    pattern = "{1} ({2})"
  ) %>%
  summary_rows(
    groups = everything(),
    columns = vars(vendors, count_org_vendors, org_vendor_perc,
                   median_org_products_count, iqr_org_products_count),
...
# create a variable to check how results would change if adding NA to org_products_count for non organic vendors
data_clean$org_products_count2 <- as.numeric(data_clean$org_products_count)
data_clean$org_products_count2[data_clean$org_vendor %in% 0] <- NA

# check how results would change
data_clean %>%
  group_by(country, city, circle) %>%
  summarise(
    median_org_products_count = median(org_products_count2, na.rm = TRUE),
    iqr_org_products_count = IQR(org_products_count2, na.rm = TRUE)
  )
```
...
# Export to table_outputs workbook
addWorksheet(table_outputs, "Table 9")
writeData(table_outputs, sheet = "Table 9", x = tab9)
saveWorkbook(table_outputs, "C:/Users/s1985751/Documents/GitHub/fea/table_outputs.xlsx", overwrite = TRUE)
```
...
### Promotional materials - Table 11
